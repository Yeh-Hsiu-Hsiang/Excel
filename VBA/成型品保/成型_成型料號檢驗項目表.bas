...
Workbooks.Open fileName:="\\yeawen\files-server\10_公用\00_i-Reporter 行動表單系統\03_設計檔案資料\品保課\成型品保\成型射出_QC檢驗紀錄表_iPad.xlsx"  '開啟檔案
    ...
    '--------貼到 IPQC FQC QRCode 檢驗紀錄表-------
    For Each ws_2 In Worksheets
        If LCase(ws_2.Name) = LCase("成型料號檢驗項目表") Then   '判斷是否已存在工作表，已存在直接複製貼上
            
            Workbooks("成型射出_QC檢驗紀錄表_QRCode_iPad.xlsx").Worksheets("成型料號檢驗項目表").Activate   '指定要上傳至iReporter檔案的活頁簿及工作表
            ...
            Workbooks("成型射出_QC檢驗紀錄表_QRCode_iPad.xlsx").Worksheets("成型料號檢驗項目表").Activate   '選擇要貼上的活頁簿、工作表
            ActiveSheet.Range("A1", Range("A1").SpecialCells(xlCellTypeLastCell)).Select '選取要貼上的範圍
            Selection.PasteSpecial  '貼上
            ActiveWorkbook.Close True   '關閉並存檔
        End If
